...
class MapProcessor(Node):
    def __init__(self):
        ...
        if self.has_parameter("clustered_map_topic"):
            self.clustered_map_topic = self.get_parameter("clustered_map_topic").value
        ...
        if self.has_parameter("cluster_selection_epsilon"):
            self.cluster_selection_epsilon = self.get_parameter("cluster_selection_epsilon").value
    ...
    def clip_points(self, points: np.ndarray) -> np.ndarray:
        ...
        points = points[(points[:, 2] > self.min_z) & (points[:, 2] < self.max_z)]
        return points
    
    def publish_clustered_map(self):
        # cluster points initially in 3D
        raw_3d = self.clip_points(self.raw_points)
        cluster_labels = self.cluster_points(raw_3d)
        cluster_labels = np.float32(cluster_labels.reshape(-1, 1))
        self.processed_points = np.concatenate((raw_3d, cluster_labels), axis=1)
        self.processed_points = self.processed_points[self.processed_points[:,3] > -1.0]
        clustered_msg = self.define_cluster_msg(self.processed_points)
    ...
    def get_map_points(self) -> Tuple[np.ndarray, np.ndarray]:
        return self.raw_points, self.processed_points
